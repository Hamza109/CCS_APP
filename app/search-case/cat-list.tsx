...
import { useCatList } from "../../src/hooks/useCat";

const CatListScreen: React.FC = () => {
  ...
  const { data, isLoading, error } = useCatList(query);
  ...
  const renderItem = ({ item }: { item: any }) => (
    <Card style={styles.card}>
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.value}>{item.respondent || "-"}</Text>
      </View>
      <View style={styles.row}>
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.value}>{item.dateoffiling || "-"}</Text>
      </View>
      {!!item.id && (
        <Button
          title='View More'
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <BlueHeader title='CAT Search Results' />
